...
# remove any unnessary stuffs when you are sure that, and parse the object with the grammar.<br/>
...
exports.andp = (items) ->
  items = for item in items
    if isMatcher(item) then literal(item) else item
  (start) ->
    cursor = start
    for item in items
...
exports.notp = (item) ->
  if isMatcher(item) then item = literal(item)
...
exports.seperatedList = (item, separator=spaces) ->
  if isMatcher(item) then item = literal(item)
  if isMatcher(separator) then separator = literal(separator)
  (start) ->
    cursor = start
    result = []
    x = item(cursor)
